...
fn transform_items(
    ...
    // First pass: collect all structs to build dependency map
    let all_structs: Vec<ItemStruct> = items
        .iter()
        .filter_map(|i| {
    ...
    items
        ...
        .map(|i| match i {
            Item::Struct(s) => Item::Struct({
                // Use comprehensive exclusion check that considers all dependencies
                let should_exclude = transformers::should_exclude_from_serde(&s, &all_structs);
                
                let s = if should_exclude {
                    // Skip all serde-related traits and transformations for problematic structs
                    transformers::add_ho_traits_struct_no_serde(&s)
                } else {
                    transformers::add_ho_traits_struct(&s)
                };
                
                let s = transformers::replace_with_shim_types(s);
                
                // Only apply serde-related transformations if struct should not be excluded
                if !should_exclude {
            ...
            Item::Enum(e) => Item::Enum({ transformers::add_ho_traits_enum(&e) }),
