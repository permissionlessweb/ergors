...
/// Check if a struct contains HashMap fields
pub fn has_hashmap_fields(s: &ItemStruct) -> bool {
    if let syn::Fields::Named(fields_named) = &s.fields {
        for field in &fields_named.named {
            if contains_problematic_type(&field.ty) {
                return true;
            }
        }
    }
...
/// Check if a struct should be excluded from serde based on its dependencies
/// This includes structs with HashMap fields OR structs that reference other non-serializable structs
pub fn should_exclude_from_serde(s: &ItemStruct, all_structs: &[ItemStruct]) -> bool {
    should_exclude_from_serde_internal(s, all_structs, &mut std::collections::HashSet::new())
}

/// Check if a type references a struct that cannot be serialized
fn references_non_serializable_struct(ty: &syn::Type, all_structs: &[ItemStruct]) -> bool {
    match ty {
        syn::Type::Path(type_path) => {
            if let Some(segment) = type_path.path.segments.last() {
                let type_name = segment.ident.to_string();
                
                // Check if this references a struct that has problematic fields
                if let Some(referenced_struct) = all_structs.iter().find(|s| s.ident.to_string() == type_name) {
                    // Use recursive check to handle transitive dependencies
                    if should_exclude_from_serde_internal(referenced_struct, all_structs, &mut std::collections::HashSet::new()) {
                        return true;
                    }
                }
                
                // Check for nested types like Option<NonSerializableStruct>, Vec<NonSerializableStruct>, etc.
                if let syn::PathArguments::AngleBracketed(args) = &segment.arguments {
                    for arg in &args.args {
                        if let syn::GenericArgument::Type(inner_ty) = arg {
                            if references_non_serializable_struct(inner_ty, all_structs) {
                                return true;
                            }
                        }
                    }
...
/// Internal recursive function to check if a struct should be excluded from serde
/// Uses a visited set to avoid infinite recursion in circular dependencies
fn should_exclude_from_serde_internal(s: &ItemStruct, all_structs: &[ItemStruct], visited: &mut std::collections::HashSet<String>) -> bool {
    let struct_name = s.ident.to_string();
    if visited.contains(&struct_name) {
        return false; // Already checking this struct, avoid infinite recursion
    }
    visited.insert(struct_name);
    
    // First check if this struct directly contains problematic fields
    if has_hashmap_fields(s) {
        return true;
    }
    
    // Then check if this struct references other structs that can't be serialized
    if let syn::Fields::Named(fields_named) = &s.fields {
        for field in &fields_named.named {
            if references_non_serializable_struct(&field.ty, all_structs) {
                return true;
            }
        }
    }
...
/// Check if a type contains HashMap or other non-serializable types
fn contains_problematic_type(ty: &syn::Type) -> bool {
    match ty {
        syn::Type::Path(type_path) => {
            // Check if the path contains HashMap (handles std::collections::HashMap, etc.)
            for segment in &type_path.path.segments {
                if segment.ident == "HashMap" {
                    return true;
                }
                
                // Check for float types that can't implement Eq
                if segment.ident == "f32" || segment.ident == "f64" {
                    return true;
                }
                
                // Check for nested types like Option<HashMap<...>>, Vec<HashMap<...>>, etc.
                if let syn::PathArguments::AngleBracketed(args) = &segment.arguments {
                    for arg in &args.args {
                        if let syn::GenericArgument::Type(inner_ty) = arg {
                            if contains_problematic_type(inner_ty) {
                                return true;
                            }
                        }
                    }
...
/// Check if a type contains HashMap (recursively checks nested types) - keeping for backward compatibility
fn contains_hashmap_type(ty: &syn::Type) -> bool {
    contains_problematic_type(ty)
}

/// Replace certain generic types with shim types for better external interfacing
pub fn replace_with_shim_types(s: ItemStruct) -> ItemStruct {
    let fields_vec = s
        .fields
        .clone()
        .into_iter()
        .map(|mut field| {
            // Replace prost_types::Struct with crate::shim::Any for better interfacing
            if field.ty == parse_quote!(::prost_types::Struct) {
                field.ty = parse_quote!(crate::shim::Any);
            } else if field.ty == parse_quote!(::core::option::Option<::prost_types::Struct>) {
                field.ty = parse_quote!(::core::option::Option<crate::shim::Any>);
            }
            // Replace prost_types::Timestamp with crate::shim::Timestamp
            else if field.ty == parse_quote!(::prost_types::Timestamp) {
                field.ty = parse_quote!(crate::shim::Timestamp);
            } else if field.ty == parse_quote!(::core::option::Option<::prost_types::Timestamp>) {
                field.ty = parse_quote!(::core::option::Option<crate::shim::Timestamp>);
            }
            // Replace prost_types::Duration with crate::shim::Duration
            else if field.ty == parse_quote!(::prost_types::Duration) {
                field.ty = parse_quote!(crate::shim::Duration);
            } else if field.ty == parse_quote!(::core::option::Option<::prost_types::Duration>) {
                field.ty = parse_quote!(::core::option::Option<crate::shim::Duration>);
            }
            field
        })
...
/// Replace certain generic types with Any type for better external interfacing (deprecated - use replace_with_shim_types)
pub fn replace_with_any_type(s: ItemStruct) -> ItemStruct {
    replace_with_shim_types(s)
}

/// Add all traits required by ho types (Serialize, Deserialize, Display for enums)
/// Skip problematic traits for structs containing HashMap fields
pub fn add_ho_traits_struct(s: &ItemStruct) -> ItemStruct {
    let mut item_struct = s.clone();
    
    let has_hashmap = has_hashmap_fields(&item_struct);

    // Only add serde traits if the struct doesn't contain HashMap fields
    if !has_hashmap {
    ...
    // Add Eq if PartialEq is present and no HashMap fields
    if !has_hashmap {
        item_struct.attrs = item_struct.attrs.into_iter().map(add_derive_eq).collect();
    }
...
/// Add minimal traits for structs that should be completely excluded from serde
/// This version skips ALL serde-related traits and Eq trait
pub fn add_ho_traits_struct_no_serde(s: &ItemStruct) -> ItemStruct {
    // Return the struct without adding any problematic traits
    s.clone()
}

/// Add all traits required by ho enums using existing dependencies
pub fn add_ho_traits_enum(e: &ItemEnum) -> ItemEnum {
    let mut item_enum = e.clone();

    // Add serde traits for all enums
    item_enum.attrs.push(syn::parse_quote! {
        #[derive(::serde::Serialize, ::serde::Deserialize)]
    });

    // Add Eq if PartialEq is present
    item_enum.attrs = item_enum.attrs.into_iter().map(add_derive_eq).collect();
...
pub fn append_attrs_struct(
    ...
    let has_problematic_fields = has_hashmap_fields(&s);

    // Skip serde traits for structs with problematic fields (HashMap, floats, etc.)
