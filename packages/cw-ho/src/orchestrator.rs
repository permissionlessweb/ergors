...
use ho_std::{
    error::error_json_detailed,
    types::ergors::orch::v1::*,
};
    ...
    match state.r.handle_request(&pr, &pr.model).await {
        Ok(llm_response) => {
            if use_open_responses {
                ...
                let response = PromptResponse {
                    id: Uuid::new_v4().into(),
                    prompt: blake3::Blake3::hash(
                        serde_json::to_string(&pr).unwrap().as_bytes(),
                    )
                    .to_string(),
        ...
        Err(e) => {
            if use_open_responses {
                ...
                Json(OpenResponsesError::ModelError {
...
pub async fn handle_open_responses(
    ...
    if let Some(ref prev_id) = prompt_req.previous_response_id {
        let prev_messages = state.s.get_open_response_context(prev_id).await.unwrap_or_default();
        if !prev_messages.is_empty() {
            debug!("Loaded {} previous messages from response {}", prev_messages.len(), prev_id);
